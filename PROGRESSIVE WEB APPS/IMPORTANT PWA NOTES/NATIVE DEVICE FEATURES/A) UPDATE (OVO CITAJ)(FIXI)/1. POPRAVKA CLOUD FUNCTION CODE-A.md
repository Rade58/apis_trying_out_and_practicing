...
### :two::cold_sweat: ISPITACU DA LI SE MOZE FIREBASE-OV storage PAKET KORISTITI NA CLOUD STRANI; A KONKRETNO STA JA ZELIM DA URADIM, JESTE DA DO ENDPOINT-A (CLUD FUNKCIJE), POSALJEM ILI Base64 STRING ILI ArrayBuffer; PA DA ISKORISTIM FIREBASE storage LIBRARY, KAKO BI UPLOAD-OVAO TAJ FAJL

***ODLUCIO SAM DA SALJEM **Uint8Array** INSTANCU (MORA DA SE DEFINISE, NJENA TRANSFORMACIJA U STRING (AKO MOZE NARAVNO))*** (PREDPOSTAVLJAM DA SAM NASO RESENJE)

- PARSING TO STRING MI TREBA ZATO STO ZELIM DA DATA SALJEM KAO JSON

- KADA SE DOBIJE NAZAD BUFFER IZ STORAGE-A (A NE VIDI MZASTO NE BI BIO U DATBASE-U) MOZE SE UZETI KAO BUFFER (BODY IMA NEKU buffer METODU); PA ONDA OD TOG BUFFER-A, PRAVIM Uint8Array, P OD Uint8 PRAVIM Blob, PA ZA Blob KORISTIM URL.CreateObjectURL
