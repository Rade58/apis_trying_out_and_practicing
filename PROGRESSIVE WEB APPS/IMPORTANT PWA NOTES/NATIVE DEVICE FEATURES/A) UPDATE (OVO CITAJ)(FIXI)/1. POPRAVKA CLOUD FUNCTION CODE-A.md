...
:two: POKUSACU DA ISPITAM DA LI SE MOZE storage PAKET KORISTITI NA CLOUD STRANI
    ...
    <script>
        ...
        dugmeZaSlanje.addEventListener('click', ev => {
            ...
            fileReaderThird.onload = e => {
                ...
                firebaseStorage.ref(`photos/${encodeURI(createdAt) + fileList[1].name}`).put(unit8);
            }
            ...
            //SAMO STO CE TRECA UCESTVOVATI PRI INSTATICIZIRANJU Unit8Array INSTANCE (PRIKAZO GORE U onload HANDLERU ) 
...
OVO, SVE GORE SAM MOGAO TESTIRATI I SA LOCALHOST-A

UGLAVNOM SVI FAJLOVI SU USPESNO BILI UPLOADED

**KADA POSMATRAS URL-OVE UPLOADED FAJLOVA, MOZES VIDETI DA JE U URL-U IMPLEMENTED uuid** (KORISCENJEM @google-cloud/ storage , TI MORAS OBEZBEDITI uuid (BAR JE TAKO BILO))

### :two::cold_sweat: ISPITACU DA LI SE MOZE FIREBASE-OV storage PAKET KORISTITI NA CLOUD STRANI; A KONKRETNO STA JA ZELIM DA URADIM, JESTE DA DO ENDPOINT-A (CLUD FUNKCIJE), POSALJEM ILI Base64 STRING ILI ArrayBuffer



***ODLUCIO SI DA SALJES **ArrayBuffer** (MORA U STRING (AKO MOZE NARAVNO))***

- PARSOVANJE STRINGA NA SERVER STRANI I UPLOADING U STORAGE

- KADA SE DOBIJE NAZAD BUFFER IZ STORAGE-A (A NE VIDI MZASTO NE BI BIO U DATBASE-U) MOZE SE UZETI KAO BUFFER (BODY IMA NEKU buffer METODU); PA ONDA OD TOG BUFFER-A, PRAVIM Unit8Array, P OD Unit8 PRAVIM Blob, PA ZA Blob KORISTIM URL.CreateObjectURL
...
- SAZNAO SAM DA ArrayBuffer MOZE BITI KORISCEN NA SERVER STRANI

- ALI [ArrayBuffer](https://javascript.info/arraybuffer-binary-arrays) IMA GOTOVO ISTI SYZE U BYTE-OVIMA (MALO JE MANJI ALI PRIBLIZNO ISTI)

ArrayBuffer NEMA NISTA SLICNO ; I KOLIKO SAM RAZUMEO ON PREDSTAVLJA MEMORY AREA, DAKLE ON JE SIROVI BINARY DATA KAO I Base64

**CAK SAM IM IZMERIO BYTE SIZE U JEDNOM PRIMERU, I ON IZNOSI GOTOVO ISTO**
...
**A POSTO FIREBASE USTVARI KORISTI GOOGLE CLOUD FUNCTIONS, ZAISTA NIJE MOGUCE SA BILO CIME STO JE NATIVNO ZA CLOUD DA SE PARSE-UJE FormData, KOJI BIA STIGAO SA CLIENT-A**

**GOOGLE CLOUD FUNCTIONS NE MOGU HANDLE-OVATI FormData VREDNOST, BEZ NEKOG DODATNO MIDDLEWEAR-A, KOJI BI PARSE-OVAO POMENUTO**

**DAKLE GOOGLE CLOUD FUNKCIJE NEMAJU 'URODJENI' NACIN DA HANDLE-UJU FAJLOVE, VEC IM TREBA NOVI PAKET, KOJI BI OPARSE-OVAO DATA IZ BODY-JA REQUEST-A**

ZASTO OVO RADIM, NA PRVOM MESTU?

ZELI MDA PROVERIM DA LI SE storage API, MOZE KORISTITI NA SERVER STRANI, ODNOSNO NA CLOUD-U; **A ZELIM DA NADJEM I NEKI ALTERNATIVNI NACIN DA SE POSALJU PODACI ODNOSNO**

- NE ZELI MDA SALJEM Blob, JR BEZ UVEZENOG PAKETA NA CLOUD-U, NE MOGU DA GA EXTRAHUJEM IZ FormData-E, A NE MOGU DA KORISTIM Blob NA SERVER STRANI

*DA KRENEM TAKO STO CU KONIGURIRATI NOVI ENDPOIT, KAKO BI OMOGUCIO CORS*

ArrayBuffer U VIDU STRING-A, SLACU NA SLEDECU VREDNSOT

**PRVO SAM ODA OMOGUCIM *CORS***

**POSTO SAM SE DO ODREDJENOG NIVOA UPOZNAO SA BINARY DAT-OM, DA SE IPAK VRATIM NA UPOTREBU @google-cloud/storage**
