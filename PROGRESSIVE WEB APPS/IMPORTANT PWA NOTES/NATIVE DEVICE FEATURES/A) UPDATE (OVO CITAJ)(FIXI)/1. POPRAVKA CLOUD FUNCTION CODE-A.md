...
exports.createPostBlah = functions.https.onRequest(async (request, response) => {
    ...
    const databsePlaceRefObjectStore = database.ref('blahPost').push();    // TAKO RECI, KADA BUDEM KORISTIO OVU REFERENCU
    ...
    const myFile = bucket.file(`blahFiles/${decodeURI(name) + fileNameExtension}`)
    ...
    try{
        ...
        const url = 'https://firebasestorage.googleapis.com/v0/b/' +
            ...
            "/o/" + "blahFiles/" + name + fileNameExtension +           // NE ZABORAVI blahFiles FOLDER DA NAVEDES
            "?alt=media&token=" + uuid;

        const id = new Date().toISOString();
        
        // NAIME, ZELI MDA ISKORISTIM OPET push METODU, KAKO BI NAPRAVIO ThenableReference ZA NOVI POST 

        const postReference = databsePlaceRefObjectStore.push(id)

        await postReference.set({     // OVO BI TTEBAL ODA GENERISE POST (I DATA U NJEMU)
                                      // U RELATED OBJECT STORE (ODNOSNO LAKACIJI)
                                      // IME OVOG POST OBJEKTA BI TREBAL ODA GENERISE METODA
                                      // ALI JA ZELI MDA ZADAM I id (JER SAM VIDEO DA JE TO NEKA PRAKS)
            title, content,
            id,
            url
        })
        ...
        const snapshot = await postReference.once('value')
