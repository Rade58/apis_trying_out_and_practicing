...
## MISLIM DA SE firebase storage LIBRARY NE MOZE KORISTITI NA CLOUD STRANI (PROVERICU OVO PA CU SE VRATITI OVDE)
...
TO MI JE MALO CUDNO (PROVERICU OVO KASNIJE, KROZ NEKI PROJEKAT, ILI PRIMER) (MISLIM DA CU TO URADITI USKORO)
...
## RESENJE CORS PROBLEMA, KOJE SE OGLEDA U TOME DA SE SAM OTREBA DEFINISATI PAR OPCIJA, U FAJLU firebase.json; I POTPUNE PROMENE URL ZA KOJ ISALJEM REQUEST (U SUSTINI TO JE DEFINISANJE REDIRECT-INGA, UNDER THE HOOD, VIDECES I KAKO)
...
>>> KADA DEPLOY-UJEM FUNKCIJE A NE KORISTIM MODERNU SINTAKSU, FIREBASE MI POKAZUJE WARNING-E, KROZ SVOJ CLI (COMMAND LINE INTERFACE) (ALI NE ZATO STO KORISTIM CommonJs MODULE, VEC ZATO STO NE KORISTIM ARROW FUNKCIJE I ZATO STO MOZDA EMBED-UJEM JEDAN PROMISE U DRUGI, UMESTO DA VRSI MCHAINING)
...
TAKODJE **OVE PAKETE BI TREBALO DA IMAM GLOBALNO** (MOZDA SAMO IPAK ADMIN-A (ALI INSTALIRAJ OBA)) (NE ZNAM ZASTO, ALI ZA SADA IH NECU INSTALIRATI GLOBALNO JER IH VEC IMAM), A UVEK ZELI MDA IH INSTALIRAM NA LATEST VERZIJU
...
const storage = admin.storage();   // !!!! IAKO GA JA USTVARI NECU KORISTITI
                                   // !!!! JER ZELIM DA KORISTIM @google-cloud/storage PAKET

// RECI CU PAR STVARI ZASTO GA NECU KORISTITI

// ZASTO GA NECU KORISTITI:

//                          POSTOJI MOGUCNOST DA SE I NE MOZE KORISTITI NA CLOUD-U
//                          ODNOSNO DA, UOPSTE (JEDNOM SAM TO U OBJASNJENJIMA GORE NAPOMENUO)
//                          A TAKODJE POSTO SE Blob NE MOZE HANDLE-OVATI NA CLOUD STRANI
//                          (ZA TO MORAM IMATI PAKET busboy)

// !!!!!!!!! UOBICAJENA PRAKSA JE UPLOADING Blob-A, PRI KORISCENJU OVOG Api-A N CLIENT STRANI
// !!!!!!!!! TADA MI JE Blob DOSTUPAN I LAK OSE MOZE INICIRATI UPLOAD (VEC POKAZO U ONOM React-OVOM PROJEKTU)

// DA LI SE, POMENUTI API MOZE KORISTITI NA CLOUD-U, JA ZELIM DA ISPITAM
// JA CU U JEDNO MNASLOVU md FAJLA, POKUSATI DA POSALJEM Base64 DATA URI DO ENDPOINT-A, I TU ZELIM DA KORISTIM
// storage NA CLOUD-U, KAKO BI POKUSAO DA TAJ Base64 DATA URI ISKORISTIM ZA UPLOADING SLIKE
// (ZAPAMTI DA SE TADA KORISTI putString METODA, NA BUCKET REFERENCI: POGLEDAJ SA OVOG  URL-A (https://firebase.google.com/docs/reference/js/firebase.storage.Reference.html#put))
// AKO TI OVO USPE, ZNACES DA MOZES KORISTITI storage API NA CLOUD STRANI
...
const storage = admin.storage(); // REAKO SAM DA CU OVO USKORO STAVITI NA TEST (NEMOJ DA ZABORAVIS)
const Busboy = require('busboy');
...
## :tea::tea::tea::tea: RANIJE SAM OBECAO DA CU ODRADITI NEKOLIKO PRIMERA, KOJI CE SE TICATI UPOTREBE FIREBASE-OVOG storage PAKETA :tea::tea::tea::tea:

:one: UPOTREBICU storage PAKET NA CLIENT STRANI, KAK OBI VIDEO KAKO JE JEDNOSTAVAN UPLOADING FAJL-OVA

:two: POKUSACU DA ISPITAM DA LI SE MOZE storage PAKET KORISTITI NA CLOUD STRANI (ODGOVOR JE VEC NE, JER ON TAMO NEMA STA DA KORISTI, JER SE TAMO NE MOZE HANDLE-OVATI Blob-OVIMA, Unit8Array-EVIMA, ILI ArrayBuffer-IMA, I ISTO VAZI I ZA Base64 URI)

### :one::dash: KREIRAM PRIMER U KOJEM NE SALJEM NISTA DO CLOUD FUNKCIJE (DAKLE NECU KREIRATI CLOUD FUNKCIJU), VEC SAMO SALJEM SLIKE U DEFAULT BUCKET FIREBASE CLOUD STORAGE-A (KORISCENJEM storage LIBRARY-JA NA CLIENT STRANI) (SAMO STO CE SVAKA OD TIH SLIAK BITI U RAZLICITOM FORMATU)
...
```HTML
    ...
    <script>
        ...
        dugmeZaSlanje.addEventListener('click', ev => {       // ZASTO JE OVO async
                                                                    // ZATO STO CE SVA TRANSFORMACIJA
                                                                    // BITI ASINHRONA
            ...
            // const fileReaderFour = new FileReader();
            // SVAKA NJENA METODA KOJA TRANSFORMISE JE USTVARI ASINHRONA
            ...
            firebaseStorage.ref().child('posts').child(decodeURI(blob.name)).put(blob)  // UPLOAD-UJEM PUTEM Blob-a

            fileReader.onload = e => {
                console.log(e.target.result)
                // UPLOAD-UJEM PUTE ArrayBuffer-a    
                firebaseStorage.ref(`photos/${decodeURI(fileList[1].name)}`).put(e.target.result)
            }
            
            fileReaderSecond.onload = e => {
                console.log(e.target.result)
            // STO SE TICE UPLODINGA FAJLA, U VIDU Base64 DATA URI-A, KORISTIM DRUGU METODU, REFERENCE BUCKET-A
            // ALI MORAM ISECI TAJ STRING UZIMAJUCI ONO IMAEDJU, ODNSONO IZMEDJU :data I IZMEDJU base64 PODSTRINGOVA
            // TU POMAZE split METODA
             firebaseStorage.ref(`photos/${decodeURI(fileList[2].name)}`).put(e.target.result.split(',')[1])
            } 
            
            fileReaderThird.onload = e => {
                console.log(e.target.result)
                // I UPLOAD-UJEM FAJL KORISCENJEM Unit8Array INSTANCE
                const unit8 =  new Uint8Array(e.target.result);
                firebaseStorage.ref(`photos/${decodeURI(fileList[1].name)}`).put(unit8);
            }
            ...
            // DAKLE SAD CU PRIMENITI METODE FileReader -A; I SVAKOJ Blob INSTANCA TREBA DA BUDE ARGUMENT
            ...
            //SAM OSTO CE TRECA UCESTVOVATI PRI INSTATICIZIRANJU Unit8Array INSTANCE 
...
OVO SAM MOGO TESTIRATI I SA LOCALHOST-A

### :two::cold_sweat: A SADA KREIRAM NOVI CLOUD FUNCTION, U KOJEM TREBAM DA HANDLE-UJEM Base64 URI (ODLUCIO SAM DA TAKODJE KORISTIM I ArrayBuffer INSTACU (JER ZELIM DA POSALJEM DVA FAJLA, KOJI CE IMATI TA DVA RAZLICITA ENCODING-A)), KOJI CE DOCI DO ENDPOINT-A, KAO POSLEDICA SLANJA 'POST' REQUEST-A, SA CLIENT-A; NAIME JA NA CLOUD STRANI ZELIM DA KORISTIM FIREBASE-OV storage LIBRARY, KAKO BI UPLOAD-OVAO FILE-OVE, OD KOJIH JE JEDAN ENCODED U VIDU ArrayBuffer INSTANCE, A DRUGI JE U VIDU Base64 DATA URI, ODNSONO STRING

**DA ODMAH KAZEM DA JE OVO JEDINO MOGUCE POSTICI, KREIRANJEM *FormData* INSTANCE**

**I ONO STO SAM GOVORIO, GOOGLE CLOUD FUNCTIONS NEMAJU NACINA DA HANDLE-UJU FAJLOVE**

**A POSTO FIREBASE USTVARI KORISTI GOOGLE CLOUD FUNCTIONS, ZAISTA NIJE MOGUCE SA BIL OCIME STO JE NATIVNO ZA CLOUD DA SE PARSE-UJE FormData, KOJI BI ASTIGAO SA CLIENT-A**



ZASTO OVO RADIM, NA PRVOM MESTU?

ZELI MDA PROVERIM DA LI SE storage API, MOZE KORISTITI NA SERVER STRANI, ODNOSNO NA CLOUD-U


*DA KRENEM OD CLIENT STRANE*

SAMO CU REDEFINISATI GORNJI PRIMER, PRI KOJEM CU ArrayBuffer SLATI NA EDNPOIT

**PRVO SAM ODA OMOGUCIM *CORS***
...
```javascript
{
  ...
  "hosting": {
    ...
    {
      "source": "/sendImageString",    // OMOGUCIO SAM DAKLE CORS KA OVOM ENDPOINT-U
...
DA DEFINISEM ,ODNOSNO REDFINISEM CLIENT CODE, IZ PREDHODNOG PRIMERA
...
<div class="blah_file_send">
    <input type="file" name="picAfile" multiple> <!-- ZADO SAM DAKLE multiple BOOLEAN ATRIBUT -->
    <button>Posalji sliku</button>               <!-- ZELIM DA SELEKTUJEM VISE FAJLOVA -->
</div>
...
<script>
        ...
        // const firebaseStorage = firebase.storage();   //  BUCKET MI VISE NE TREBA OVDE JER NE ZELI MDA KORISTIM
                                                         // storage API NA CLIENT STRANI

        dugmeZaSlanje.addEventListener('click', ev => {

            if(pikerSlike.files[0] instanceof Blob && ){

                // UZIMAM Blob
                const blob = pikerSlike.files[0];

                // KREIRAM NOVU FileReader INSTANCU
...
**POSTO SAM SVE TO VIDEO, HAJDE DA SE VRATIM NA UPOTREBU @google-cloud/storage**
...
[POGLEDAJ I OVAJ MEDIUM-OV CLANAK]() (DEV, KOJI JE, ISTO PRAVIO PROJEKAT ZA FIREBASE PALTFORMU, USPESNO JE KORISTIO POMENUTI, TUTORIJAL, I USPESNO DEFINISAO FUNKCIJU, KOAJ HANDLE-UJE MULTIPART FORM DATA)

****

**DIGRESIJA**

NAJBOLJE BI BILO DA URADIM JEDAN PRIMER, PRATECI, TAJ TUTORIJAL, IZ GOOGLE CLOUD FUNCTIONS DOKUMENTACIJE

U MOM APP-U , ZADACU JEDAN TEMPORARY ELEMENT, KOJI CU KASNIJE UKLONITI, A KOJI SE SASTOJI OD FILE PICKER-A, I SASTOJI SE OD SUBMIT DUGMET-A, I JOS JEDNOG INPUT FIELD-A
