...
<script>
    ...
    const fileInp = document.querySelector('div.blahElem > input[type="file"]');
    const buttSend = document.querySelector('div.blahElem > button');
    const titleInp = document.querySelector(
        'div.blahElem > input:not(:nth-of-type(1)):not(:nth-last-of-type(1))'
    )
    ...
    buttSend.addEventListener('click', async e => {
        ...
        console.log({mimeType, fileName, content, title});
...
exports.sendFo = functions.https.onRequest(async (request, response) => {
    ...
    if(!request.method === 'POST') return response.status(405).send("this is not POST");
    ...
    busboy.on('file', (fieldname, file, filename) => {
        ...
        tempPathsForFiles[fieldname] = filepath;
    ...
    busboy.on('finish', () => {
        ...
        Promise.all(processPromisses)
        .then(() => {

            for(let fieldname in tempPathsForFiles){

                const filepath = tempPathsForFiles[fieldname]

                console.log(filepath)
            }

            return response.status(200).send('files are written')
