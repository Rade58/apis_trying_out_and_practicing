...
exports.createPostBlah = functions.https.onRequest(async (request, response) => {
    ...
    try{
        ...
        const snapshot = await databasePlaceRef.once('value')   // MOZDA NISAM NI MORAO KORISTITI SNAPSHOT VEC SAM MOGAO
                                                                // PUTEM key-A DA PRISTUPIM POST-U
...
POMENUTI PRIMER JE BIO USPESAN ALI JA SAM KORISTIO BINARY DATA

JOS DA NAPOMENEM DA SU MOJI FAJLOVI, UVEK BILI UPLOADED KAO FAJLOVI TIPA

**application/octet-stream**

STO MOGU VIDETI U KONZOLI GDE JE PRIKAZAN MOJ UPLOADED FAJL

OVO MI BAS I NE IZGLEDA DOBRO, JER PREDPOSTAVLJAM DA KADA JE PROCESSUJEM FAJL DA BIH OD NJEGA DOBIO NJEGOVE BINARNE PODATKE, DA SE NESTO OD VALUABLE PODATAKA 'GUBI'
...
POGOTOVO MI JE SJAJNA [ThenableReference INSTANCA](https://firebase.google.com/docs/reference/js/firebase.database.ThenableReference)

POTREBNO JE DAKLE DA PRODJES OPET SVE FEATURE-E FIREBASE-A

****

## ZATO CU SADA KONZNO ODRADITI JEDAN PRIMER U KOJEM CU KORITITI MIDDLEWEAR U MOM PROJEKTU, A TO CE BITI busboy KOJI CE SLUZITI ZA PARSING MULTIPART FORM PODATAKA, A KAO STO ZNAM BEZ MIDDLEWEAR-A, SE NE MOGU PROCESSOVATI MULTIPART FORM DATA, JER CLOUD NEMA SUPPORT ZA TO

POSTOJI NEKOLIKO 'AKTUELNIH' MIDDLEWEAR-A

- [busboy](https://www.npmjs.com/package/busboy) KOJI CU KORISTITI I ZA KOJI SAM USPEO DA MI PROFUNKCIONISE, JER MOZDA SAM RANIJE NAPRAVIO NEKE GRESKE (ZATO SAM IMAO UTISAK DA busboy NE RADI DOBRO, ALI IPAK SAM USPESNO DEFINISAO JEDAN PRIMER KOJI CU SADA POKAZATI)

- [multer](https://www.npmjs.com/package/multer)

- [multiparty](https://www.npmjs.com/package/multiparty#readme)

- [formidable](https://www.npmjs.com/package/formidable) (SIGURNO NE RADI NA CLOUD-U)
...
<div class="blahElem">
    ...
    <input type="text" placeholder="Title">
    <input type="text" placeholder="Content">
    <button>send form, together with two pictures</button>
    <section></section>
</div>
<script>

    const fileInp = document.querySelector('div.blahElem > input[type="file"]:first-of-type');
    const fileInp2 = document.querySelector('div.blahElem > input[type="file"]:nth-of-type(2)');
    const buttSend = document.querySelector('div.blahElem > button');
    const titleInp = document.querySelector(
        'div.blahElem input[placeholder=Title]'
    )
    const contentInp = document.querySelector('div.blahElem input[placeholder=Content]')



    buttSend.addEventListener('click', async e => {

        const content = contentInp.value.trim(); 
        const title = titleInp.value.trim();
        const blob = fileInp.files[0];
        const blob2 = fileInp2.files[0];
        
        const fileName = blob.name;
        const fileName2 = blob2.name;
        ...
        formData.append("file2", blob2, fileName2);
        ...
        try{
            ...
            response = await fetch('https://instapwaclone.firebaseapp.com/sendFo',
                {
                    method: "POST",
                    /* headers: {
...
U SUSTINI PRATIO SAM SLEDECE [OBJASNJENJE ZA USING MULTIPART FORM DATA](https://cloud.google.com/functions/docs/writing/http#multipart_data) SA FIREBASE CLOUD FUNKCIJAMA

JA CU SADA MALO DRUGACIJE DEFINISATI UPLOAD, NEGO U ONOM PRIMER-U IZ PROSLE LEKCIJE, SADA CU KORISTITI, [upload METODU Bucket-A](https://googleapis.dev/nodejs/storage/latest/Bucket.html#upload)

```javascript
exports.sendFo = functions.https.onRequest(async (request, response) => {

    // PRE NEGO STO POCNEM, RECI CU DA CU JA busboy-u DODATI rawBody REQUEST-A
    // KAK OBI GA ON PARSE-OVAO
    // ALI PRE NEGO STO MU GA PROSLEDIM, MORACU DA DEFINISEM SVE EVENT LISTNER-E 


    // A AKO NIJE REC O 'POST' REQUEST-U, SALJEM ERROR SA STATUSOM  405
    // DA DRUGI METOD NIJE DOZVOLJEN

    if(!request.method === 'POST') return response.status(405).end("this is not POST");

    // ZASTO SE KORISTI end NISAM BAS SIGURAN, IMA NEKIH OBJASNJENJA, ALI NISTA NECU DODAVATI 
    // DODATNO DOK DOBRO NE NAUCIM Node.js
    // U SUSTINI MOGU RECI DA END-UJEM RESPONSE PROCES SA POSLATIM ERROR-OM
    ...
    //              -  IME FIELD-A, KOJE JE ZADATO, ONDA KADA JE Blob APPENDED NA FormData INSTANCU (NA CLIENT-U)
    //              -  CELI PATH DO FAJLA, U TEMPORARY FOLDER-U OPERATIVNOG SISITEMA CLOUD-A
    const tempPathsForFiles = {};
    ...
    // I TAJ NIZ, KASNIJE TREBA DA BUDE ARGUMENT,     Promise.all()        METODE
    ...
    busboy.on('file', (fieldname, file, filename, encoding, mimeType) => {
        ...
        tempPathsForFiles[fieldname] = {filepath, mimeType};  // VIDECES DA CE TI PRILIKOM UPLOAD-A, TREBATI I mimeType

        const writable = file_system.createWriteStream(filepath);

        // !!!!!!!!!!!!!!!!    CISTO DA NAPOMENEM       file        PARMAETAR SE ODNOSI NA READABLE STREAM

        
        // SADA UCITAVAM FAJL (READABLE) U TAJ WRITE STREAM, ALI NECU TO URADITI PRE NEGO STO ZAKACIM NEKE EVENT HANDLERE
        // I NA WRITABLE, ALI I NA FILE

        // file.pipe(writable);   // OVO JE MOGLO DA BUDE OVDE (NISAM IMAO NIKAKAV ERROR, KAO POSLEDICU
                                  // ALI RAZMISLIO SAM DA BI BIL ODOBRO DA KACIM HANDLERE, KOJI SE TICU 
                                  // OBOG PROCESA, PRE NEGO STO SE ZAPOCNE PROCES)

        
        

        // MEDJUTIM MISLIM DA IMAM BOLJE OBJASNJENJE KOJE SAM PRONASAO IZ OVOG TEJKSTA
        ...
        // (OVO JE MALO PREVISE DETALJNO ZA Node.js POCETNIKA, ZATO CU DOLE DATI RAZUMLJIVIJE OBJASNJENJE)
        ...
        const promise = new Promise((resolve, reject) => {
            ...
            file.on('end', () => { 
                writable.end()    // KADA ZOVEM end NA WRITABLE-U
                                  // TRIGGER-OVACE SE       'finish'        ILI     'error'     NA TOM WRITABLE-U    
                

                // A KADA JE, USTVARI ONDA OVAJ   'end'    EVENT TRIGGERED NA READABLE-U?
                
                    // The 'end' event is emitted when there is no more data to be consumed from the stream.
                    //      https://nodejs.org/api/stream.html#stream_event_end

                // DAKLE KADA SE EVENT 'end' TRIGGER-UJE NA READBALEU (JER VISE NEMA PODATAKA ZA PROCESS-OVANJE)

                // JA ZELIM DA DA NA WRITABLE-U PRIMENIM end METODU

            //      https://nodejs.org/api/stream.html#stream_writable_end_chunk_encoding_callback
            // >>>>>   Calling the writable.end() method signals that no more data will be written to the Writable

                // A KAO POSLEDICA TOGA SE NA WRITABELU TREIGGER-UJU ILI 'finish' ILI 'error' EVENT

                // STO JE CONVINIENT ZA MENE I MOGU resolve-OVATI ILI reject-ovati, PROMISE, U ODNSU NA TO

                // IMAJ NA UMU DA JE OVO DEFINISANO ZA SVAK IDETECTED FAJL

            })

        })

        // EVO OVDE SAM ZADA OPOMENUTI 'PIPING', MADA KAO STO SAM I REKAO, MOGAO SAM TO I RANIJE
        file.pipe(writable);

        // DODAJEM promise U NIZ            (DAKLE, ZA SVAKI DETECTED FAJL BICE PO JEDAN PROMISE U OVOM NIZU)
        processPromisses.push(promise);
    ...
    // ONO STA MI PADA NAPAMENT, KADA CE SE TO TRIGGER-OVATI OVAJ 'finish' EVENT NA busboy-U
    
    //          PREDPOSTAVLJAM ONDA KADA JE READABLE STREAM, MOG REQUESTA,  POTPUNO UPOTREBLJEN OD STRANE busboy-A
    //      A TO BI BILO ONDA KADA JE       'end'       TRIGGERED NA REDABLE-U

    // ZATO SE OVDE PAMENTNO KORISTI Promise.all METODA

    // PREDPOSTAVLJAM, DA SE ZATO STO TREBAM BITI SIGURAN DA JE SVE UPISANO U WRITABLE STREAM-OVE ZA SVAKI FAJL;
    // JA USTVARI VODIM RACUNA, DA SE POZVALO end NA WRITABLE-U
    // KOJEG CU DAKLE KORISTITI (NJEGOV PATH), KAKO BI UPLOAD-OVAO FAJLOVE U GOOGLE CLOUD STORAGE


    //*********************************************** */
    // ALI PRE NEGO STO POCNEM SA DEFINISANJEM on 'finish' EVENT HANDLER-A
    // KAKO BI IZBEGAO TOLIKI NESTING PROMISE-A, ZELIM DA KREIRAM JEDAN DODATNI PROMISE, KOJI CE BITI
    // VAN OBIMA, POMENUTOG EVENT HANDLER-A (KASNIJE CE MI POSTATI JASNO ZASTO SAM KREIRAO TAJ PROMISE BAS OVDE)
    // USTVARI TREBA MI PROMISE KOJI CE BITI IZVAN OBIMA busboy-EVOG finish HANDLER-A
    // A ZELEIM DA SE TAJ PROMISE RESOLVE-UJE SA PODACIMA SA KOJIMA MOGU RECREATE-OVATI DOWNLOAD URL-OVE, 

    // A ONO STA TAKODJE ZELIM DA URADIM, JESTE DA SACUVAM resolve CALLBACK U JOS JEDNOJ VARIJABLI

    let resolveFunction;

    let uploadFinishPromise = new Promise((resolve, reject) => {

        resolveFunction = resolve;

        // DAKLE ZA POCETAK OVAJ PROMISE JE PENDING
        // A AKDA SE ON RESOLVE-UJE (DEFINISACU DA TO BUDE, KADA SE SVI FAJLOVI UPLOAD-UJU)
        // JA ZELIM DA OTPOCNEM UNOS PODATAKA U DATBASE

    })

    //******************************************************************* */

    // OVDE CE MI TREBATI, MOJ BUCKET LOCATION
    // A LI IPAK DA PRIPREMIM I SVE KOMPONENTE DOWNLOAD LINK-A
    const baseDownloadPart = 'https://firebasestorage.googleapis.com/v0/b/';
    const bucketLocation = 'gs://instapwaclone.appspot.com';
    const oPart = "/o/";
    const queryString = "?alt=media&token=";
    
    /* const url = 'https://firebasestorage.googleapis.com/v0/b/' +
    bucketLocation.replace("gs://", "") +
    "/o/" + "blahFiles" + encodeURIComponent("/") + encodeURIComponent(name) + fileNameExtension +
    "?alt=media&token=" + uuid;
    */
    ...
    busboy.on('finish', () => {
        ...
        Promise.all(processPromisses) // OVO JE ONAJ PROMISE, KOJI SE RESOLVE-UJE KADA FINISH-IRA WRITABLE STREAM

        .then(() => {

            let urlsBuildParts = [];        // OVDE ZELIM DA STAVIM PROMISE ZA SVAKI UPLOAD,
                                            // ALI TEK ONDA KADA DOGODI UPLAOD, PROMISE TREBA DA BUDE RESOLVED

            for(let fieldname in tempPathsForFiles){

                // ZATO ZA SVAKI UPLOAD KREIRAM PROMISE, KOJI SE TREBA RESOLVE-OVATI TEK KAD SE ZAVRSI KOMPLETAN UPLOAD
                // SVIH FAJLOVA

                let resolveUploadFunction;

                // PROMISE STAVLJAM U GORNJI NIZ
                urlsBuildParts.push(new Promise((resolve, reject) => {

                    resolveUploadFunction = resolve; 

                }))


                // DAKLE, ZA SVAKI FAJL
                // SADA CU DA KORISTIM ONAJ MIME TYPE I PATH FAJLA U TEMP FOLDERU OS-A

                let filepath = tempPathsForFiles[fieldname]["filepath"];
                let mimeType = tempPathsForFiles[fieldname]["mimeType"];

                // KREIRAM PATH NA KOJEM FAJL TREBA BITI UPLOADED U BUCKET-U
                // OBRATI PAZNJU NA RegExp
                // JA SAM DEFINISAO DA OVAJ REGEXP USTVARI PRONADJE '/deo patha-a/' ILI 'deo path-a/'
                let destination = `folderBlah/${filepath.replace(/\/*\w+\//g,"")}`;   
                
                console.log({filepath, mimeType, destination})

                // ZA SVAKU SLIKU PO JEDAN uuid
                let uuid = UUID();

                // AKO ZELIS DA UPLOAD-UJES NA SPECIFICNOJ LOKACIJI, KORISTI destination PROPERTI U OPTIONS
                // ARGUMENTU

                bucket.upload(
                    filepath,       // PATH FAJLA U TMP FOLDERU OS-A
                    {
                        // NAPOMINJEM DA destination NEMA VEZE SA DOWNLOAD URL-OM (TO BI TREBALO DA TI BUDE JASNO)
                        // ALI IMA VEZE SA ONIM GDE JE FAJL SMESTEN, U KOJI FOLDER NA 
                        //                                              gs://instapwaclone.appspot.com
                        destination,
                        ...
                        metadata: {
                            metadata: {
                                contentType: mimeType,
                                firebaseStorageDownloadTokens: uuid        // NARAVNO ZADA JEM uuid
                            }
                        }
                    ...
                    (err, uploadedFile) => {
                        ...
                        // OVO UPRAVO ZNACI DA KADA SE JEDAN FAJL UPLOAD-UJE JA RESOLVE-UJEM PROMISE
                        resolveUploadFunction({
                            uuid, destination, fieldname 
            ...
            // A PROMISE RELATED TO UPLAOD FAJLA, JE STAVLJEN U NIZ KOJO JE ARGUMENT Promise.all POZIV-A

            // JA ZELIM DAKLE DA PROSLEDJUJEM SVE GRADIVNE ELEMENTE URL-A, ZA SVAKI FAJL, ONOM resolve-U
            // ONOG DRUGOG PROMISE-A, 'SPOLJASNJEG PROMISE-A, KAKO BI IMAO S CIM DA SAGRADIM URL-OVE KASNIJE

            // ALI TO SE TEK RADI KADA SE IZVRSI USPESNO OVAL Promise.all
            // ODNOSNO KADA SE RESOLVE-UJE SA NIZOM ONIH GRADIVNIH ELEMENATA ZA DOWNLOAD URL-OVE, SVIH FAJLOVA,
            // KOJI SU UPLOADED I CIJI DOWNLOAD URL, ZELI MDA RECREATE

            return Promise.all(urlsBuildParts)

        })
        .then(urlsBuildParts => {

            // EVO OVDE RESOLVE-UJEM TAJ SPOLASNJI PROMISE
            return resolveFunction(urlsBuildParts);
        })
        .catch((err) => {

            console.log({errorIsThat: err})
    ...
    busboy.end(request.rawBody);    // EVO, OVDE SAM BUSBOY-A, 'NAHRANI-O', REQUEST-OVIM rawBody-Jem
    // I OND POCINJE PROCESS PARSING-A, I TOKOM TOGA SE TRIGGER-UJU, ONI EVENT HANDLERI, KOJE SAM PREDHODNO
    // ZAKACIO

    // STO SE TICE, POMENUTOG SPOLJASNJEG PROMISE-A, POMENUTI PROMISE KADA SE RESOLVE-UJE, ON TREBA DA BUDE 
    // 'OKIDAC' ZA POPUNJAVANJE DATABASE PODACIMA JEDNOG POSTA. MEDJU KOJIM SU DOWNLOAD URL-OVI, I FIELD-OVI,
    // KOJE JE IZ REQUEST-A, TAKODJE OBEZBEDIO, PRSE-UJUCI rawBody busboy 

    return uploadFinishPromise.then(async urlsBuildParts => {  // OVO MOZE DA BUDE I ASYNC FUNKCIJA, JER MR MRZI DA DALJE NSTUJEM PROMISE-E

        // DA POCNEM SA GRADNJOM SVAKOG URL-A

        let dataObject = {}

        for(let object of urlsBuildParts){

            // RE KREIRAM URL ZA SVAKI FAJL (MNOGE ELEMENTE SAM VEC PRIPREMIO)
            // NEMOJ NIKAD DA ZABORAVIS DA ENCODE-UJE URI COMPONENT, KOJI SE ODNOSI NA FOLDER U BUCKETU, I IMAE FAJLA
            // JER TAJ DEO JE ENCODED AKO SADA POGLEDAS KONZOLU I TAMO UPLOADED FAJAL
            let url = baseDownloadPart + bucketLocation.replace("gs://", "") + oPart +
            encodeURIComponent(object.destination) + queryString + object.uuid;
            
            dataObject[object.fieldname] = url;

        }

        // DODAJEM I fields-OVE

        Object.assign(dataObject, fields)

        // return response.status(200).json(dataObject)        // MALA PROVER DA VIDIM DA LI SU SVI PODACI TU
    
        // OVDE CU OBAVITI DEFINISANJE POPULATING-A DATBASE LOKACIJE, SA JEDNIM POST-OM, NAPRAVLJENI MSA
        // DATOM, KOAJ JE OVDE PROSLEDJENA

        // 'OBJECT STORE', KOJI ZELIM DA IMAM ZVACE SE 'bablengStore'  

        const bablengStoreRef = database.ref('bablengStore')
        
        const bablengPostRef = bablengStoreRef.push()     // push CE ZNACITI DA CE POST BITI KREIRAN
                                                                        // U POMENUTOM OBJECT STORE-U
                                                                        // POD GENERISANIM ID-JEM
        // ALI JOS NISAM ZAPOCEO STORING
        // TO MORAM URADITI METODOM set
        
        await bablengPostRef.set(
           dataObject 
        )

        const snapshot = await bablengPostRef.once('value')         // SADA ZELIM DA PROCITAM PODATKE POST-A
                                                                    // I ZATO TRAZIM DataSnapshot
        
        // EVO OVDE MOGU KONACNO POSLATI PODATKE, SA KOJIMA SE NA FRONT END-U, MOZE DEFINISATI
        // KORISCENJE TIH PODATAKA, MEDJU KOJIMA SU DVA STRINGA (title I content) I DVA URL-A (ZA file I file2)

        return response.status(200).json(snapshot.val())


        // MOZDA JE OVO ONO STO SAM SPOMINJAO NEKADA A STO SE ZOVE 'OPTIMISTIC UPDATE'? MOZDA?
...
AKO TESTIRAM GORNJI CODE VIDECU DA SVE FUNKCIONISE, TAKO DA SADA KONACNO MOGU ISKORISTITI BUSBOY I GOOGLE CLOUD STORAGE U SLUCAJU, MOG PRIMERA, ODNOSNO MOG PWA-A

## :dash::dash::dash::dash: SADA MOGU ODPOCETI SA DEFINISANJEM storePostData CLOUD FUNKCIJE, KOJA CE BUKVALNO BITI SVA BACKEND MOJE PWA APLIKACIJE; ALI NA ONOVU NAUCENOG SADA MOGU USPESNO DEFINISATI OVU FUNKCIJU:dash::dash::dash::dash:

MOZDA PREVISE INFORMACIJA JER MISLIM DA JE TO SASVIM JASNO

SADA CU GLEDATI DA STO MANJE OBJASNJAVAM I STO VISE PISEM CODE

GORE NEGDE U TEKSTU U NEKOM NASLOVU PRIKAZAO SAM

- RELATED FRONTEND CODE

- SECURITY PRAVILA ZA DATBASE, ALI I ZA STORAGE (ZA SADA JE SVE PERMISSIVE, ZATO STO IPAK VEZBAM)

- KAKO DA OMOGUCIM CORS BEZ IKAKVOG MIDDLEWEAR-A (firebase.json FAJL)

- A STO JE NAJVAZNIJE SADA ZNAM DO ODREDJENOG NIVO KAKO FUNKCIONISE BACKEDND (fs, path, os PAKETI, WRITABLE-OVI ,READABLE-OVI)

****

MISLIM DA SAM O OVOME VEC PRICAO ALI NEKA OSTANE OVDE
...
- A TAKODJE SAM SE UPOZNAO KAKO TO request.rawBody BIVA PARSED BY BUSBOY

- I NAUCIO SAM DO ODREDJENOG NIVOA [@google-cloud/storage API](https://googleapis.dev/nodejs/storage/latest/index.html) (TU ZELI MI DA DODAM DA SAM NAUCIO I File DEO)

SADA CU POCETI SA DEFINISANJEM CLOUD FUNKCIJE, ALI CU OPET POKAZATI KOJE SAM SVE PAKETE UVOZIO, I KAK OSAM INICIJALIZOVAO APP I TAKO BLIZE I TAKO DALJE
...
const storage = admin.storage();      // VEC MI JE POZNATO DA JE OVO GOOGLE CLOUD STORAGE
...
//***************PROMENI NA LAZNE KAD BUDES KOPIRAO OOVO NAZAD U md FAJL */ 
const webpush = require('web-push');
const privateVapidKey = "oC8DBLahIEnXnAMEzbrVY";
const publicVapidKey = "BMfwxEIIhs40w8CDWGn-vS_m122NEFkIgcDWDRgFzvrBCChNxX_spJoSM";
//******************************** */
...
exports.storePostData = functions.https.onRequest(async (request, response) => {
    ...
    const pathsTempAndMime = {};

    const writingStreamFinishPromises = [];
    ...
    busboy.on('file', (fieldname, fileStream, filename, encoding, mimeType) => {
        ...
        const filePathTemp = path.join(tempDir, filename);

        pathsTempAndMime[fieldname] = {filePathTemp, mimeType};

        // NE ZABORAVI DA PATH GDE JE FAJ TRENUTNO, DODAS KAAO ARGUMENT
        const writeStream = file_system.createWriteStream(filePathTemp);

        const writingFinishPromise = new Promise((resolve, reject) => {
        ...
        writingStreamFinishPromises.push(writingFinishPromise);

        fileStream.pipe(writeStream);

    })

    // OVO RADIM NAMERNO DA BIH DEFINISAO PROCES UPLOADINGA (I OSTALO), IZVAN ON  finish   HANDLER-A
    // JA TO USTVARI SAMO RADIM DA MI CODE IZGLEDA CLEANER I DA IKORISTIM TO, STO SAM DEFINISAO DA 
    // MOJA CLOUD FUNKCIJA, USTVARI BUDE async FUNKCIJA
    // CODE U KOJEM SU await IZJAVE IZGLEDA ZNATNO CLEANER

    let resolveFunction;

    const uploadStartPromise = new Promise((resolve, reject) => {
        resolveFunction = resolve;
    })
    ...
    busboy.on('finish', () => {
        ...
        Promise.all(writingStreamFinishPromises)
        .then(() => {
    ...
    await uploadStartPromise;


    const allUploadsFinishPromises = [];

    const bucketName = 'instapwaclone.appspot.com'

    try{

        const bucket = storage.bucket('gs://' + bucketName)

        for(let fieldname in pathsTempAndMime){

            let uuid = UUID();

            let {filePathTemp, mimeType} = pathsTempAndMime[fieldname];

            let destination = `postImages/${filePathTemp.replace(/\/*\w+\//g, "")}`;

            allUploadsFinishPromises.push(
                new Promise((resolve, reject) => {
                    ...
                    bucket.upload(
                        ...
                        (err, file) => {
                            ...
                            if(file) resolve({fieldname, destination, uuid});
    ...
    const filesUploadData = await Promise.all(allUploadsFinishPromises);
    ...
    for(let object of filesUploadData){

        dataObject[object.fieldname] = baseDownloadPart + bucketName + oPart + 
                                        encodeURIComponent(object.destination) + queryString + object.uuid;
    ...
    await postRef.set(dataObject);

    return response.status(201).send(`Post is created: id--> ${dataObject.id}`)
...
MEDJUTIM POGRESIO SAM  UJEDNOM

JA SAM GORE DEFINISAO DA SE I URL FAJLA STORE-UJE, POD ISTIM IMENOM, KAO I ONAJ FIELDNAME, POD KOJIM SAM NA CLIENT-U PRE SLANJA POSTA, APPEND-OVAO BLOB NA FormData

MISLIM DA MI JE LAKSE DA ISPRAVIM FRONT END, NEGO DA DIRAM PREDHODNU FUNKCIJU, KOJA JE REUSABLE, ODNOSNO MOZE SE KORISTITI ZA BILO KOJI FORM DATA

NAIME, POSTO SE DOWNLOAD URL UPLOADED SLIKE, CUVA POD IMENOM image, JA CU DEFINISATI DA SE POD TIM IMENOM APPEND-UJE Blob NA FormData INSTANCU

public/src/js/feed.js

```javascript
const sendData = function(){    // FALLBACK FUNKCIJA AKO NIJE PODRZAN BACKGROUND SYNC ILI SERVICE WORKER
...
KADA DEPLOY-UJEM VIDECU DA CE SADA SVE FUNKCIONISATI KAKO TREBA

## ALI AKO SE SECAS OD RANIJE, OVU TVOJU FUNKCIJU JE CINIO I DEO CODE-A, KOJI SE ODNOSIO NA WEB PUSH

ZA SADA CU TAJ CODE SAMO PREKOPIRATI, ODNOSNO IPAK CU GA REDEFINISATI TOLIKO DA KORISTI await SINTAKSU, TIME CE IZGLEDATI CLEANER

A STO SE TICE OSTALIH STVARI VEZANIH ZA MOJ PWA, PA ONE SVE ZAHTEVAJU TEMELJAN RELEARNING, ALI TO CU URADITI TAKO STO CU NAKON ZAVRSETKA BAVLJENJA SA SVIM TUTORIJALIMA VEZANIM ZA PWA, JA IZGRADITI NEKU APLIKACIJU, KOJA CE KORISTITI PWA TEHNOLOGIJE

CISTO DA NAPOMENEM DA POSTOJI I [CLOUD MESSAGING](https://firebase.google.com/docs/cloud-messaging/js/client) KOJI SE IMPLEMENTIRA UZ POMOC **FIREBASE**-A

SECAS SE DA POSTOJE SERVERI KOJI SE BAVE I KOJI SU VENDOR BASED (VLASNICI SU KOMPANIJE KOJE PRAVE BROWSER-E) (MOZDA FIREBASE KORISTI DRUGACIJU TEHNOLOGIU, ISPITAJ TO)

**DA NAPOMENEM, DA SAM JA U MOM PRIMERU KORISTIO [web-push](https://www.npmjs.com/package/web-push) PAKET, A CIJA [OBJASNJENJA MOGU NACI OVDE](https://github.com/web-push-libs/web-push#web-push)**

A SADA POSVETI SE PRIMERU
...
//***************DAKLE ISKORISTICU IH SADA */ 
const webpush = require('web-push');
const privateVapidKey = "6Yd-NAjHvWJrZYkxv1X";
const publicVapidKey = "BHsK51yqgSlSy48ZAbRojIXk-xW62x7yngTeXQ48sZ2eI";
//******************************** */


/* const gcsConfig = {keyFilename: '/instaclone-fb-key.json', projectId: 'instapwaclone'}  // VISAK
const {Storage} = require('@google-cloud/storage');
const storage = new Storage(gcsConfig); */
...
// DAKLE DOLE U CODE-U, KADA SE ZAVRSI SA POPUNJAVANJEM DATBASE-A, SA DATA-OM, JEDNOG POST-A
// DEFINISE SE WEB PUSH RELATED CODE
...
exports.storePostData = functions.https.onRequest(async (request, response) => {
    ...
    //******************** */ ODAVDE CU DAKLE ODPOCETI SA WEBPUSH-OM  //****************************** */

    webpush.setVapidDetails('mailto:bajic.rade2@gmail.com', publicVapidKey, privateVapidKey);

    // TREBA MI SNAPSHOT ONOG 'subscriptions' MESTA U REAL TIME DATBASE-U

    const snapshot = await database.ref('subscriptions').once('value');

    // SVAKOM SUBSCRIPTIONU PRISTUPAM, KAKO BI MOGAO POSLATI NOTIFIKACIJU, SVAKOM OD SUBSCRIBED CLIENATA

    // let sendNotifiPromises = [];   // OVO MI NECE TREBATI JER NE ZELIM DA
    // DA BIL OSTA AWAIT-UJEM
    // ODNOSNO STA AKO IMAM MILION SUBSCRIBED CLIENTS-A, PA TADA BI FUNKCIA OTISLA U TIMEOUT
    // AKO AWAIT-UJEM Promise.all()

    snapshot.forEach(sub => {

        // let resolvement;    // OVO COMMENTED OUT SE ODNOSI NA OVO GORE STO SAM NAPISAO

        /* sendNotifiPromises.push(

            new Promise((resolve, reject) => { resolvement = resolve })
     ...
     */
        ...
        pushConfig["endpoint"] = value.endpoint;
        pushConfig["keys"] = {};
        pushConfig["keys"]["auth"] = value.keys.auth;
        pushConfig["keys"]["p256dh"] = value.keys.p256dh;

        const payload = JSON.stringify({
            title: `New post: ${dataObject.title}`,
        ...
        // PAZI DA OVDE ARGUMENTE NE STAVIS U OBJECT (TAK OSAM JEDNOM POGRESIO, I CONFIG I PAYLOAD STAVIO U OBJEKAT ARGUMENT)
        webpush.sendNotification(pushConfig, payload)
        .then(() => {
            // resolvement()
            return console.log('notification is sent')
        })
        .catch(err => {

            console.log(err)

            response.status(500).send("Notification is not sent!!!")
        })
    ...
    // await Promise.all(sendNotifiPromises)
...
STO SE TICE [CLIENT SIDE CODE](https://github.com/Rade58/apis_trying_out_and_practicing/blob/master/PROGRESSIVE%20WEB%20APPS/IMPORTANT%20PWA%20NOTES/PUSH%20NOTIFICATIONS/PUSH/g%29%20POBOLJASNJE%20MOG%20CODE.md) NJEG ASAM KAO STO ZNAS DEFINISAO ONDA KADA SAM SE VAVIO SA PUSH

SADA SE KONACNO MOGU VRATITI SA DALJIM BAVLJENJEM NATIVE DEVICE FEATURE-IMA; A ONO CIME CU SE SLEDECE BAVITI JESTE LOCATION NATIVE DEVICE FEATURE, ODNOSNO [GETTING USER LOKLACIJE](https://github.com/Rade58/apis_trying_out_and_practicing/blob/master/PROGRESSIVE%20WEB%20APPS/IMPORTANT%20PWA%20NOTES/NATIVE%20DEVICE%20FEATURES/8.%20USER%20LOCATION.md#user-location), ALI TAKODJE SAM ZABORAVIO DA IMPLEMENTIRAM [FALLBACK, ZA SLUCAJ KADA KORISNIK NIJE DAO PERMISSION ZA KAMERU ILI DEVICE NEMA KAMERU](https://github.com/Rade58/apis_trying_out_and_practicing/blob/master/PROGRESSIVE%20WEB%20APPS/IMPORTANT%20PWA%20NOTES/NATIVE%20DEVICE%20FEATURES/7.%20FILE%20PICKER.md#sada-zelim-da-definisem-fallback-koji-treba-da-bude-implemented-ako-app-nema-pristupa-kameri-bilo-da-je-to-iz-razloga-sto-korisnik-nije-dao-permission-ili-sto-na-uredjaju-nema-kamere)
