...
exports.sendFo = functions.https.onRequest(async (request, response) => {
    ...
    // KAKO BI IZBEGAO TOLIKI NESTING PROMISE-A, ZELIM DA KREIRAM JEDAN DODATNI PROMISE, KOJ ICE BITI
    // VAN OBIMA, POMENUTOG EVENT HANDLER-A (KASNIJE CE MI POSTATI JASNO ZASTO SAM KREIRAO TAJ PROMISE BAS OVDE)

    // A ONO STA TAKODJE ZELI MDA URADIM, JESTE DA SACUVAM resolve CALLBACK U JOS JEDNOJ VARIJABLI
    ...
    let uploadFinishPromise = new Promise((resolve, reject) => {
        ...
        // (MEDJU TIM PODACIMA, BICE I DOWNLOAD URL-O)
    })
    ...
    busboy.on('finish', () => {
        ...
        // OVDE CE MI TREBATI, MOJ BUCKET
        const bucketLocation = 'gs://instapwaclone.appspot.com';
        const bucket = storage.bucket(bucketLocation);

        Promise.all(processPromisses) // ZASTO ZELIM DA IMAM OVAJ PROMISE NAPOLJU
                                                              // SHAVTICES KADA VIDIS CIME TREBA DA SE
                                                              // RESOLVE-UJE, SLEDECE then
        .then(() => {
            ...
            for(let fieldname in tempPathsForFiles){
                ...
                // NAKON STO SU SVI WRITABLE-I SPREMNI, MOGU SE DAKLE POZABAVITI
                // KORISCENJEM LINKOVA FAJLA ()
                // KOJE CU ISKORISTITI ZA UPLOAD

                // DAKLE ZA SVAKI FAJL, KOJI JE I DALJE NA SVOJOJ TEMP LOKACIJI U TEMP FOLDERU OS-A GCS (GOOGL CLOUD FUNKCIJA)
                // JA RADIM SLEDECE
                ...
                // JASNO TI DA SI ZADAO OVDE SAMO IME
                // ALI MORAO SAM ISECI ONAJ 
                // PREDNJI DEO PATHA DA BI DOBIO SAMO IME
                ...
                // AKO ZELIS DA UPLOAD-UJES NA SPECIFICNOJ LOKACIJI, KORISTI location PROPERTI U OPTIONS
                // ARGUMENTU

                bucket.upload(
                    filepath,       // PATH FAJLA U TMP FOLDERU OS-A
                    {
                        // NAPOMINJEM DA location NEMA VEZE SA DOWNLOAD URL-OM (TO BI TREBALO DA TI BUDE JASNO)
            ...
            // PROSLEDJUJEM SVE GRADIVNE ELEMENTE URL-A, ZA SVAKI FAJL, ONOM resolve-U
            // ONOG SPOLJASNJEG PROMISE-A, KAKO BI IMAO SCI MDA SAGRADIM URL-OVE KASNIJE
    ...
    busboy.end(request.rawBody);    // NARAVNO, EVO GDE SAM     busboy-U DAO rawBody DA GA KORISTIM


    uploadFinishPromise.then(urlsBuildParts => {

        const responseObject = {urlsBuildParts, fields}
...
ISTO TAKO PROBAJ I [multiparty](https://www.npmjs.com/package/multiparty#readme)
